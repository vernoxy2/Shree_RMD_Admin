...
import { MdDashboard, MdLogout, MdLocalHospital } from "react-icons/md";
  ...
  return (
    <div className="flex min-h-screen bg-[#F0F4F8] font-sans">
      <aside
        className={`${
          collapsed ? "w-16" : "w-64"
        } bg-[#1A3C5E] text-white flex flex-col transition-all duration-300 ease-in-out relative`}
      >
        <button
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center sticky top-0 z-10">
          <div>
            <h2 className="text-base font-semibold text-gray-800">Hospital Admin Dashboard</h2>
            <p className="text-xs text-gray-400">RMD Ayurveda College & Hospital</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#1A3C5E] text-white flex items-center justify-center text-sm font-bold">
              H
            </div>
            <span className="text-sm text-gray-600">Hospital Admin</span>
          </div>
        </header>
