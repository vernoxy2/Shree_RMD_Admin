...
const HospitalAttendance = () => {
  ...
  const colRef = collection(db, "hosp_attendance");
  const BUCKET_NAME = "hospital-attendance-files";
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const sortedData = data.sort((a, b) => {
        if (b.year !== a.year) return b.year.localeCompare(a.year, undefined, { numeric: true });
  ...
  const handleSubmit = async () => {
    if (!form.year) return showAlert("Year is required.", "warning");
    if (!form.month) return showAlert("Month is required.", "warning");
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "hosp_attendance", editId), updateData);
  ...
  const handleDelete = async (rec) => {
    showConfirm(
      `Are you sure you want to delete this record for ${rec.month} ${rec.year}? This action cannot be undone.`,
      async () => {
        try {
          if (rec.filePath) {
            await supabase.storage.from(BUCKET_NAME).remove([rec.filePath]);
          }
          await deleteDoc(doc(db, "hosp_attendance", rec.id));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[#1A3C5E]">Hospital Attendance</h1>
          <p className="text-sm text-gray-500 mt-1">Manage hospital biometric attendance by month and year</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border border-gray-100">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5 ml-1">Month *</label>
                  <select
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A3C5E] bg-gray-50/50"
                    value={form.month}
              ...
              <div>
                ...
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-[#1A3C5E] hover:bg-blue-50/30 transition-all group">
                  ...
                  <span className="text-xs font-medium text-gray-500 text-center px-4">
                    {file ? file.name : "Drop biometric register file here"}
                  </span>
                  <input
                    type="file"
                    className="hidden"
        ...
        <div className="space-y-10">
          {sortedYears.map((year) => (
            <div key={year} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-2xl font-black text-[#1A3C5E] mb-4 ml-1">{year}</h2>
              
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-xl shadow-gray-200/50">
                {/* Table Header */}
                <div className="bg-[#1A3C5E] text-white px-6 py-4 grid grid-cols-[60px_1fr_120px] sm:grid-cols-[80px_1fr_200px] items-center font-bold text-sm tracking-wide">
                  <span>Sr.</span>
                ...
                <div className="divide-y divide-gray-100">
                  {groupedRecords[year].map((rec, idx) => (
                    <div
                      key={rec.id}
                      className="px-6 py-5 grid grid-cols-[60px_1fr_120px] sm:grid-cols-[80px_1fr_200px] items-center hover:bg-blue-50/30 transition-colors group"
                    >
                      <div className="flex items-center">
                        <span className="w-8 h-8 rounded-full bg-blue-50 text-[#1A3C5E] flex items-center justify-center text-xs font-black">
                          {idx + 1}
                        </span>
                      </div>
