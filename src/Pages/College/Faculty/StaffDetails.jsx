...
const StaffDetails = () => {
  ...
  const colRef = collection(db, "cllg_staff_details");
  const BUCKET_NAME = "staff-files";

  const staffTypes = ["Teaching Staff", "Non-Teaching Staff", "Hospital Staff"];
  ...
  const handleSubmit = async () => {
    ...
    if (!form.staffType) return showAlert("Staff Type is required.", "warning");
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "cllg_staff_details", editId), updateData);
  ...
  const handleEdit = (rec) => {
    setForm({ 
      year: rec.year,
      staffType: rec.staffType || "Teaching Staff"
    });
    setEditId(rec.id);
    setShowForm(true);
  };
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          await deleteDoc(doc(db, "cllg_staff_details", rec.id));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Staff Details</h1>
          <p className="text-sm text-gray-500 mt-1">Manage teaching, non-teaching, and hospital staff</p>
        </div>
        <button
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border border-gray-100">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5 ml-1">Staff Type *</label>
                  <select
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#7B1C2E] bg-gray-50/50 appearance-none"
          ...
          {sortedYears.map((year) => (
            <div key={year} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-xl shadow-gray-200/50">
                {/* Table Header */}
                <div className="bg-[#7B1C2E] text-white px-6 py-4 grid grid-cols-[60px_1fr_120px_120px] sm:grid-cols-[80px_1fr_150px_200px] items-center font-bold text-sm tracking-wide">
                  <span>Sr.</span>
                ...
                <div className="divide-y divide-gray-100">
                  {groupedRecords[year].map((rec, idx) => (
                    <div
                      key={rec.id}
                      className="px-6 py-5 grid grid-cols-[60px_1fr_120px_120px] sm:grid-cols-[80px_1fr_150px_200px] items-center hover:bg-rose-50/30 transition-colors group"
                    >
                      ...
                      <div className="text-center">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                          rec.staffType === "Teaching Staff" ? "bg-blue-50 text-blue-600" :
                          rec.staffType === "Non-Teaching Staff" ? "bg-purple-50 text-purple-600" :
