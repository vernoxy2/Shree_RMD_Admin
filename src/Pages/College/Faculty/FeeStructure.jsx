...
const FeeStructure = () => {
  ...
  const colRef = collection(db, "cllg_fee_structure");
  const BUCKET_NAME = "fee-files";
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "cllg_fee_structure", editId), updateData);
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          await deleteDoc(doc(db, "cllg_fee_structure", rec.id));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Fee Structure</h1>
          <p className="text-sm text-gray-500 mt-1">Manage college fee structure by year</p>
        </div>
        <button
