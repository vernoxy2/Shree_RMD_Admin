...
const UniversityResults = () => {
  ...
  const colRef = collection(db, "cllg_university_results");
  const BUCKET_NAME = "University-Results-files";
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        ...
        await updateDoc(doc(db, "cllg_university_results", editId), updateData);
        ...
        showAlert("New result added successfully!");
      }
      resetForm();
      fetchRecords();
    } catch (err) {
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          await deleteDoc(doc(db, "cllg_university_results", rec.id));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">University Results</h1>
          <p className="text-sm text-gray-500 mt-1">Manage and publish examination results</p>
        </div>
        <button
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1.5 ml-1">Result Title *</label>
                  <input
                    ...
                    placeholder="e.g. BAMS First Year Result March 2026"
                    value={form.title}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          ...
          <span className="text-sm font-medium text-gray-400">Loading results...</span>
        </div>
      ) : records.length === 0 ? (
        <div className="text-center py-32 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
          ...
          <p className="text-gray-500 font-medium">No results found. Start by adding one.</p>
        </div>
      ) : (
